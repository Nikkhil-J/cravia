    ...
    <div className="flex-1">
      ...
      {selected ? (
        <div className="rounded-xl border border-primary bg-primary-light p-3">
          <p className="text-sm font-medium text-heading">{selected.name}</p>
...
export default function ComparePage() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10">
      <h1 className="font-display text-2xl font-bold text-heading">Compare dishes</h1>
      ...
      <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:gap-4">
        ...
        />
        <div className="flex items-center justify-center py-1 text-sm text-text-muted sm:pt-5 sm:py-0">vs</div>
        <DishPicker
          slot={2}
          query={query2}
          setQuery={setQuery2}
        ...
        <div className="mt-8 overflow-hidden rounded-2xl border border-border">
          ...
          <div className="hidden bg-bg-cream sm:grid sm:grid-cols-3">
            <div className="p-4 text-center">
              {dish1.coverImage && <Image src={getOptimizedImageUrl(dish1.coverImage, 'thumbnail') ?? ''} alt={dish1.name} width={80} height={80} className="mx-auto h-20 w-20 rounded-lg object-cover" />}
            ...
            <div className="p-4 text-center">
              {dish2.coverImage && <Image src={getOptimizedImageUrl(dish2.coverImage, 'thumbnail') ?? ''} alt={dish2.name} width={80} height={80} className="mx-auto h-20 w-20 rounded-lg object-cover" />}
          ...
          <div className="flex items-center gap-3 bg-bg-cream p-4 sm:hidden">
            <div className="min-w-0 flex-1 text-center">
              {dish1.coverImage && <Image src={getOptimizedImageUrl(dish1.coverImage, 'thumbnail') ?? ''} alt={dish1.name} width={56} height={56} className="mx-auto h-14 w-14 rounded-lg object-cover" />}
            ...
            <div className="min-w-0 flex-1 text-center">
              {dish2.coverImage && <Image src={getOptimizedImageUrl(dish2.coverImage, 'thumbnail') ?? ''} alt={dish2.name} width={56} height={56} className="mx-auto h-14 w-14 rounded-lg object-cover" />}
          ...
          {/* Rating rows — 3-col on sm+, stacked cards on mobile */}
          {[
            { label: 'Overall', v1: dish1.avgOverall, v2: dish2.avgOverall },
            { label: SUB_RATING_LABELS[0], v1: dish1.avgTaste, v2: dish2.avgTaste },
            { label: SUB_RATING_LABELS[1], v1: dish1.avgPortion, v2: dish2.avgPortion },
            { label: SUB_RATING_LABELS[2], v1: dish1.avgValue, v2: dish2.avgValue },
          ].map(({ label, v1, v2 }) => (
            <div key={label} className="border-t border-border">
              <div className="hidden px-4 py-3 sm:grid sm:grid-cols-3">
              ...
              <div className="flex items-center justify-between px-4 py-3 sm:hidden">
                ...
                <div className="flex items-center gap-3">
                  <span className={`text-sm font-semibold ${v1 > v2 ? 'text-primary' : 'text-text-primary'}`}>{formatRating(v1)}</span>
                  <span className="text-[10px] text-text-muted">vs</span>
                  <span className={`text-sm font-semibold ${v2 > v1 ? 'text-primary' : 'text-text-primary'}`}>{formatRating(v2)}</span>
                </div>
              </div>
            </div>
          ))}
          ...
          <div className="border-t border-border">
            ...
            <div className="flex items-center justify-between px-4 py-3 sm:hidden">
              <span className="text-xs font-medium text-text-muted">Reviews</span>
              <div className="flex items-center gap-3">
                <span className="text-sm text-text-secondary">{dish1.reviewCount}</span>
                <span className="text-[10px] text-text-muted">vs</span>
                <span className="text-sm text-text-secondary">{dish2.reviewCount}</span>
              </div>
            </div>
          </div>
        </div>
